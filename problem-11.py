...
def tokenizer(x, split_on='\n'):
    """Split string by a given character value."""
    return [i for i in x.split(split_on) if len(i) > 0]



def xsum(iterable):
    """Sum elements of an iterable."""
    n = 0
    for i in iterable:
...
def xlen(iterable):
    """Length (character count) of an iterable."""
    return xsum([1 for i in iterable])
...
if __name__ == '__main__':
    ...
    #-- Create our matrix
    matrix = reformat_matrix(raw_matrix)

    #--Variable to collect output of maximum product in the matrix.
    max_value = 0
    ...
    while True:
        ...
        boxes = list()
        for r in range(xlen(matrix) - 1):
            row = sm_matrix[r]
            for c in range(xlen(matrix[r]) - 1):
                #-- Create a temp list
                #-- Extend each vector section into the list
                tmp = list()
                tmp.extend(matrix[r][c:c+2])
                tmp.extend(matrix[r+1][c:c+2])
                if not tmp in boxes:
                    boxes.append(tmp)

        #-- Continue with evaluating the product of all the 2x2 boxes
        for idx, box in enum(boxes):
            xbox = chunker(box)
            max_product = max([xproduct(chunk) for c_idx, chunk in enum(xbox) if not 0 in chunk])
            if max_product > max_value:
                max_value = max_product
        break
